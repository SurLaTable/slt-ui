        ...
        if (/\//.test(path.node.source.value)) {
          ...
          if (
            ...
            path.node.specifiers[0].local.name.slice(0, 1) ===
              path.node.specifiers[0].local.name.slice(0, 1).toUpperCase()
          ) {
            ...
            const firstSpecifier = path.node.specifiers[0].local.name;

            path.node.specifiers[0].local.name = `{ ${firstSpecifier}`;
            const lastSpecifier = path.node.specifiers[path.node.specifiers.length - 1].local.name;
            path.node.specifiers[path.node.specifiers.length - 1].local.name = `${lastSpecifier} }`;
          }
        }
      }
      return path;
